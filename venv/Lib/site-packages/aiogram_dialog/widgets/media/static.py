...
class StaticMedia(Media):
    ...
    async def _render_media( self,data: Any, manager: DialogManager) -> Optional[MediaAttachment]:
        return MediaAttachment(
            type=self.type,
            url=self.url,
            # string=self.string.format_map(data),
            string=self.string,
            path=self.path.format_map(data),
            # path=self.path,
            **self.media_params,
        )
